...
VariableNode* DenseLayer(VariableNode* input, int width, Graph& graph,
  ...
  VariableNode* weight_matrix = Operations::Constant(Tensor{weight_shape, 0.}, final_name + "_weight", graph);
  
  // The bias is of size [width, 1]
  Tensor::shape_type bias_shape = {(uint32_t)width};
  VariableNode* bias = Operations::Constant(Tensor{bias_shape, 0.},
    final_name + "_bias", graph);
  
  VariableNode* wx = Operations::Mul(weight_matrix, input, final_name + "_matmul", graph);
  VariableNode* wxb = Operations::Add(wx, bias, final_name + "_addbias", graph);
  VariableNode* layer_value = Operations::ReLU(wxb, final_name + "_relu", graph);
